                    ...
                    if (args.length != 2) {
                        ...
                        if (args[1] == 'cm') {
                            ...
                            yahooFinance.quote({
                                ...
                                const Embed = new DiscordJS.MessageEmbed()
                                    .setTitle(`${PriceSymbol}/${PriceSymbol2}`)
                                    .setDescription(`${quoteType}`)
                ...
                case 'historial':
                    ...
                    if (args.length != 3 || (args[2] != 'd' && args[2] != 'w' && args[2] != 'm' )) {
                        ...
                        if (args[1] == 'cm') {
                            ...
                            fulldate = fulldate[0] + '-' + fulldate[2] + '-' + fulldate[1]
    
                            var topen = [], tclose = [], tdate = []
                            var open = [], close = [], date = []
                            ...
                            yahooFinance.historical({
                                ...
                                for (let i = 0; i < res.length; i++) {
                                    ...
                                    var resobj = restext.split('\n')
                                    for (let i = 0; i < resobj.length; i++) {
                                        if (resobj[i].includes('date')) {
                                            var obj = resobj[i].split(':')
                                            var out = obj[1].split('T')
                                            tdate.push(out[0].substring(2,12))
                                        }
                                        else if (resobj[i].includes('open')) {
                                            var obj = resobj[i].split(':')
                                            let lobj = obj[1].length
                                            topen.push(obj[1].substring(1, lobj - 1))
                                        }
                                        else if (resobj[i].includes('close')) {
                                            var obj = resobj[i].split(':')
                                            let lobj = obj[1].length
                                            tclose.push(obj[1].substring(1, lobj - 1))
                                        }
                                    }
                                }
    
                                for(var i = 0;  i < 24; i++) {
                                    if (tclose[i] != 'null' && topen[i] != 'null') {
                                        date.push(tdate[i])
                                        topen[i] = (parseFloat(topen[i]) * pq)
                                        open.push(topen[i])
                                        tclose[i] = (parseFloat(tclose[i]) * pq)
                                        close.push(tclose[i])
                                    }
                                }
                                date.reverse()
